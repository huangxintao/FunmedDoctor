    ...
    protected void init() {
        ...
        if (getFragmentManager() != null) {
            getFragmentManager().beginTransaction().replace(ResFinder.
                    getResourceId(ResFinder.ResType.ID, "id_content"), parseJson.ttc.size()==0?topicBaseFragments.get(2):topicBaseFragments.get(0)).commit();
        }
    }
...
//                transaction.replace(ResFinder.getResourceId(ResFinder.ResType.ID,"id_content"),categoryFragment).commit();
    ...
    public void initFragment() {
        if (parseJson.ttc.size() == 0){
            topicBaseFragments.add(new FocusedTopicFragment());
            topicBaseFragments.add(new RecommendTopicSearchFragment());
            topicBaseFragments.add(new CategoryFragment());
        }else {
